...
public class BTreeFile implements DbFile {
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		BTreeLeafPage newRightTreeLeafPage = (BTreeLeafPage) getEmptyPage(tid, dirtypages, BTreePageId.LEAF);
		int NumTuples = page.getNumTuples();
		Iterator<Tuple> reversIterator = page.reverseIterator();

		for (int i = 0; i < NumTuples / 2; i++){
			Tuple tuple = reversIterator.next();
			page.deleteTuple(tuple);
			newRightTreeLeafPage.insertTuple(tuple); 
		}

		BTreePageId rightSiblingId = page.getRightSiblingId();
		BTreeLeafPage rightTreeLeafPage = null;
		if (rightSiblingId != null){
			rightTreeLeafPage = (BTreeLeafPage) getPage(tid,dirtypages,rightSiblingId,Permissions.READ_ONLY);
		}

		if (rightTreeLeafPage != null){
			rightTreeLeafPage.setLeftSiblingId(newRightTreeLeafPage.getId());
			newRightTreeLeafPage.setRightSiblingId(rightTreeLeafPage.getId());
			dirtypages.put(rightTreeLeafPage.getId(), rightTreeLeafPage);
		}

		page.setRightSiblingId(newRightTreeLeafPage.getId());
		newRightTreeLeafPage.setLeftSiblingId(page.getId());
		dirtypages.put(page.getId(), page);
		dirtypages.put(newRightTreeLeafPage.getId(), newRightTreeLeafPage);
		
		Field midKey = newRightTreeLeafPage.iterator().next().getField(keyField);
		BTreeEntry insertEntry = new BTreeEntry(midKey, page.getId(), newRightTreeLeafPage.getId());
		...
		if (midKey.compare(Op.GREATER_THAN, field)){
			return page;
		}
		else{
			return newRightTreeLeafPage;
		}		
	}
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		BTreeInternalPage newRighTreeInternalPage = (BTreeInternalPage) getEmptyPage(tid,dirtypages,BTreePageId.INTERNAL);
		...
		for (int i = 0; i < numEntries / 2; i ++){
			...
			newRighTreeInternalPage.insertEntry(entry);
		}

		BTreeEntry midEntry = reverseIterator.next();
		page.deleteKeyAndRightChild(midEntry);
		midEntry.setLeftChild(page.getId());
		midEntry.setRightChild(newRighTreeInternalPage.getId());

		BTreeInternalPage parentTreeInternalPage = getParentWithEmptySlots(tid,dirtypages,page.getParentId(), midEntry.getKey());
		parentTreeInternalPage.insertEntry(midEntry);
		
		dirtypages.put(page.getId(), page);
		dirtypages.put(newRighTreeInternalPage.getId(), newRighTreeInternalPage);
		dirtypages.put(parentTreeInternalPage.getId(), parentTreeInternalPage);

		updateParentPointers(tid, dirtypages, page);
		updateParentPointers(tid, dirtypages, newRighTreeInternalPage);
		updateParentPointers(tid, dirtypages, parentTreeInternalPage);

		if (midEntry.getKey().compare(Op.GREATER_THAN, field)){
			return page;
		}
		else{
			return newRighTreeInternalPage;
		}
	}
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		int midnumTuples = (pagenumTuples + siblingnumTuples) / 2;

		for(int i = pagenumTuples; i < midnumTuples; i++){
			Tuple siblingTuple = siblingIterator.next();
			sibling.deleteTuple(siblingTuple);
			page.insertTuple(siblingTuple);
		}

		Tuple headSibling = siblingIterator.next();
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// some code goes here
		Iterator<BTreeEntry> leftInternalIterator = leftSibling.reverseIterator();
		BTreeEntry leftEntry = leftInternalIterator.next();
		BTreeEntry oldParentEntry = new BTreeEntry(parentEntry.getKey(), leftEntry.getRightChild(), page.iterator().next().getLeftChild());
		page.insertEntry(oldParentEntry);

		int pagenumEntries = page.getNumEntries();
		int siblingnumEntries = leftSibling.getNumEntries();
		int midnumEntries = (pagenumEntries + siblingnumEntries) /2;

		for (int i = pagenumEntries; i < midnumEntries; i++){
			leftSibling.deleteKeyAndRightChild(leftEntry);
			page.insertEntry(leftEntry);
			leftEntry = leftInternalIterator.next();
		}

		BTreeEntry newParentEntry = leftEntry;
		leftSibling.deleteKeyAndRightChild(newParentEntry);
		parentEntry.setKey(newParentEntry.getKey());
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		Iterator<BTreeEntry> rightInternalIterator = rightSibling.iterator();
		BTreeEntry rightEntry = rightInternalIterator.next();
		BTreeEntry oldParent = new BTreeEntry(parentEntry.getKey(), page.reverseIterator().next().getRightChild(), rightEntry.getLeftChild());
		...
		for (int i = pagenumEntries; i < midnumEntries; i++){
			...
			rightEntry = rightInternalIterator.next();
		}

		BTreeEntry newParent = rightEntry;
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		while(rightIterator.hasNext()){
			Tuple nexTuple = rightIterator.next();
			rightPage.deleteTuple(nexTuple);
			leftPage.insertTuple(nexTuple);
		}
		...
		if(newrightSiblingId == null){
			...
			BTreeLeafPage newRightLeafPage = (BTreeLeafPage) getPage(tid, dirtypages, newrightSiblingId, Permissions.READ_WRITE);
			newRightLeafPage.setLeftSiblingId(leftPage.getId());
		}
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// Delete the entry in the parent corresponding to the two pages that are merging -
		// deleteParentEntry() will be useful here
		Iterator<BTreeEntry> rightInternalIterator = rightPage.iterator();

		BTreeEntry oldparentEntry = new BTreeEntry(parentEntry.getKey(), leftPage.reverseIterator().next().getRightChild(), rightPage.iterator().next().getLeftChild());
		leftPage.insertEntry(oldparentEntry);

		while(rightInternalIterator.hasNext()){
			BTreeEntry nextEntry = rightInternalIterator.next();
