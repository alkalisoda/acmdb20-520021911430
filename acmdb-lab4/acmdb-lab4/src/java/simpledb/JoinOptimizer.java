...
public class JoinOptimizer {
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        if (joinOp == Predicate.Op.EQUALS){
            ...
            double double_card = 0.3 * card1 * card2;
            card = (int) double_card;
        }
    ...
    public Vector<LogicalJoinNode> orderJoins(
        ...
        for (int i = 0; i < joins.size() + 1; i++){
            Set<Set<LogicalJoinNode>> nodeSets = enumerateSubsets(joins, i);
            for (Set<LogicalJoinNode> nodeSet : nodeSets){
                ...
                if(bestCostSoFar != Double.MAX_VALUE){
                    planCache.addPlan(nodeSet, bestCostCard.cost, bestCostCard.card, bestCostCard.plan);
                }
            }
        }
        if(explain){
